...
const PLATFORMS: Record<SiteLinkKey, PlatformMeta> = {
  spotify: {
    label: "Spotify",
    action: "Escuchar Perfil",
    category: "music",
...
export function EnlacesSection({ site }: SiteTemplateProps) {
  const profile = site.profile;
  const activeKeys = SITE_LINK_KEYS.filter((key) => Boolean(profile.links[key]));
  const hasLinks = activeKeys.length > 0;

  return (
    <section
      id="enlaces"
      data-section="enlaces"
      ...
      {!hasLinks ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {activeKeys.map((key, index) => {
            ...
            return (
              <SiteReveal key={key} delay={index * 0.05} yOffset={16}>
                <a
                  href={href}
                  target="_blank"
                  rel="noreferrer"
                ...
                >
                  ...
                  {/* Bottom Info: Name & Direct Action */}
                  <div className="mt-6">
