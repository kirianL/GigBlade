...
export function CustomerListTable({
	...
	const isFullQueryPending =
		(isFullCustomersLoading || isFullCustomersFetching) &&
		...
		return customers.map((customer) => {
			...
			return {
				...
				...(fullCustomer
					...
					: {}),
				fullCustomer,
				isFullDataLoading: !fullCustomer && isFullQueryPending,
			} as CustomerWithProducts;
		});
	}, [customers, fullCustomersMap, isFullQueryPending]);
		...
		<Table.Provider
			config={{
				...
				virtualization: {
					...
					skeletonRowCount: Math.max(customers.length, 5),
					rowHeight: 40,
				},
			}}
		>
