...
describe("product list customer count loading", () => {
	test("keeps the Customers cell accessible while counts load", () => {
		const cell = renderCustomerCell({ isCountsLoading: true });

		expect(cell.props.children.type).toBe("span");
		expect(cell.props.children.props.className).toBe("sr-only");
	});
