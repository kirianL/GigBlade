			...
			{COUNTS.map((n) => (
				<button
					...
					className={cn(
						...
						value === n
							? "border-brand-glow bg-brand-glow/15 text-white"
...
export function RunPill({
	...
	return (
		<button
			...
			onClick={onClick}
			disabled={disabled}
			className="relative inline-flex items-center gap-1 overflow-hidden rounded-md border border-brand bg-brand px-2 py-1 font-sans text-[12px] font-medium text-white shadow-[0_1px_2px_rgba(0,0,0,0.4)] transition duration-200 after:pointer-events-none after:absolute after:inset-0 after:bg-[linear-gradient(135deg,rgba(255,255,255,0.14),transparent_55%)] hover:bg-brand active:scale-[0.98] disabled:opacity-50"
		>
			<svg
				width="8"
				height="8"
...
export function SectionLabel({
	...
	return (
		<div
			className={cn(
				"bg-[#141414] px-4 py-1.5 font-mono text-[11px] uppercase tracking-[0.06em] text-brand-glow",
				className,
			)}
		>
			{children}
