	...
	return (
		...
		>
			<div className="flex items-center gap-2 border-b border-[#292929] px-3 py-1.5">
				<span className="text-brand-glow">{table.icon}</span>
				<span className="font-mono text-[11px] uppercase tracking-[0.06em] text-brand-glow">
					{table.name}
				</span>
			</div>
								...
								return (
									<td
										className={cn(
											...
											flashing ? "text-brand-glow" : "text-[#E5E5E5]",
										)}
										key={key}
									>
										{value}
				...
				connectors={CONNECTORS.map((conn) => {
					...
					return (
						<g key={key}>
							...
							{!reduce && (
								<circle fill="var(--color-brand-glow)" r={3}>
									<animateMotion
										dur="1.6s"
