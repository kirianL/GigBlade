...
export function Connector({
	...
	const color = active ? "var(--color-brand-glow)" : "#2c2c2c";
	return (
	...
	return (
		<div
			className={cn(
				...
				active
					? "border-brand-glow bg-brand-glow/8 shadow-[0_0_0_1px_var(--color-brand-glow)]"
		...
		>
			<span
				className={cn(
					...
					active ? "bg-brand-glow" : "bg-[#3a3a3a]",
				)}
			/>
			<div className="flex items-center gap-2">
				{icon && (
					<span
						className={cn(
							"shrink-0",
							active ? "text-brand-glow" : "text-[#FFFFFF66]",
						)}
					>
						{icon}
					</span>
