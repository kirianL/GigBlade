			...
			<div className="max-w-[820px] mx-auto px-4 xl:px-0">
				<header className="mb-12">
					...
					<p className="text-[14px] md:text-[16px] leading-6 text-[#FFFFFF99] font-light font-sans max-w-[640px]">
						...
						<AppLink
							href="/llms.txt"
							className="text-brand-glow hover:text-brand-soft underline underline-offset-2"
						>
							llms.txt
						</AppLink>{" "}
						and{" "}
					...
					{groups.map((group) => (
						<section key={group.category}>
							<h2 className="flex items-center gap-2 font-mono text-[13px] md:text-[14px] uppercase tracking-[-1%] text-white mb-5">
								<span className="w-[8px] h-[8px] bg-brand-glow" />
								{group.category}
							</h2>
									...
									<li key={doc.slug}>
										...
										>
											<div className="flex items-baseline justify-between gap-4">
												<span className="font-sans text-[16px] md:text-[18px] tracking-[-2%] text-white group-hover:text-brand-soft transition-colors duration-300">
													{doc.title}
												</span>
